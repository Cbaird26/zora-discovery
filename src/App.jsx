...
import { branchProbabilities, buildSymmetricFutures, timelineBranchProbabilities } from "./lib/math.js";
...
function DiscoveryTab({ discoveryMode, ...props }) {
...
export const TOP_LEVEL_DEVICES = [
  ...
  { id: "decision", label: "Decision", color: P.green, comp: DiscoveryTab, discoveryMode: "DECISION", discovery: true },
  { id: "intent", label: "Intent", color: P.glow3, comp: DiscoveryTab, discoveryMode: "INTENT", discovery: true },
  { id: "navigation", label: "Navigation", color: P.glow, comp: DiscoveryTab, discoveryMode: "NAVIGATION", discovery: true },
  { id: "research", label: "Research", color: P.gold, comp: DiscoveryTab, discoveryMode: "RESEARCH", discovery: true },
];
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: P.void, color: P.text, fontFamily: FONT, position: "relative", overflow: "hidden" }}>
      <div style={{ position: "relative", zIndex: 1, padding: "max(18px, env(safe-area-inset-top)) max(14px, env(safe-area-inset-right)) max(24px, env(safe-area-inset-bottom)) max(14px, env(safe-area-inset-left))" }}>
        ...
        <div style={{
          ...
          zIndex: 10,
          background: isMobile ? "linear-gradient(180deg, rgba(6,7,12,0.95), rgba(6,7,12,0.76))" : "transparent",
          backdropFilter: isMobile ? "blur(14px)" : "none",
          borderRadius: isMobile ? 14 : 0,
        }}>
          {TOP_LEVEL_DEVICES.map(d => (
            <button type="button" key={d.id} onClick={() => applySelection(d.id)} style={{
              minHeight: 44,
        ...
        <div style={{ marginTop: 16, textAlign: "center" }}>
          <div style={{ fontSize: 11, color: P.faint, letterSpacing: "0.04em", lineHeight: 1.7 }}>
            {"GROUNDING: Probability tilt \u00B7 Yukawa fold preview \u00B7 GKSL branch selection \u00B7 Decision / Intent / Navigation / Research bridge modes"}
          </div>
          <div style={{ fontSize: 11, color: P.dim, marginTop: 5, fontStyle: "italic" }}>
            {'"A working compilation surface matters because it shows the thought experiment, the bridge, and the product view in one place." \u2014 C. M. Baird, 2026'}
          </div>
        </div>
      </div>
